...
if (isset($_POST['email']) && isset($_POST['password'])) {
  ...
  if ($email !== "" && $password !== "") {
       ...
       // on regarde si le mail et le pwd existe dans la table badgeuse
      $requete = "SELECT count(*) FROM badgeuse where 
    email = '" . $email . "' and password = '" . $pass . "' ";
      ...
      $req2 = "SELECT */*'badgeuse_role'*/ FROM roles where 
      email_id = '" . $email_id . "'";
      $exec_req2 = mysqli_query($db, $req);
      $rep2 = mysqli_fetch_array($exec_req);
      $badgeuse_role = $rep2['badgeuse_role'];

      

    if ($count != 0) {
      $_SESSION['badgeuse_role'] = $badgeuse_role;
      $_SESSION['badgeuse'] = true;
      if ($_SESSION['badgeuse_role'] == '1000') {
       header('Location: public/badgeuse_admin.php');

      } else if ($_SESSION['badgeuse_role'] == '1') {
        header('Location: public/badgeuse_user.php');
      } else echo "role inconnu";
    } else {


      header('Location: public/badgeuse.php?erreur=1'); // utilisateur ou mot de passe incorrect
    }
  } else {
    header('Location: public/badgeuse.php?erreur=2'); // utilisateur ou mot de passe vide
  }
} 
...
// si l'utilisateur est deja log sur le Workspace
    ...
    if(isset($_SESSION['badgeuse_role']))
    {
      if ($_SESSION['badgeuse_role'] == '1000') 
      {
       header('Location: public/badgeuse_admin.php');
